...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run tests for EEP1")
    parser.add_argument(
        "--test-type", 
    ...
    # Set environment variable for service URLs if not set
    if "IEP1_URL" not in os.environ:
        os.environ["IEP1_URL"] = "http://localhost:5001"
    if "IEP2_URL" not in os.environ:
        os.environ["IEP2_URL"] = "http://localhost:5004"
    if "IEP3_URL" not in os.environ:
        os.environ["IEP3_URL"] = "http://localhost:5003"
    if "IEP4_URL" not in os.environ:
        os.environ["IEP4_URL"] = "http://localhost:5005"
