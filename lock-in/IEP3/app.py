...
metrics.info("app_info", "IEP3 Google Calendar service", version="1.0.0")

# Create custom metrics
api_requests = Counter(
    'api_requests_total', 
    'Total number of requests to IEP3 API',
    ['method', 'endpoint', 'status']
)

api_duration = Histogram(
    'api_request_duration_seconds', 
    'Duration of IEP3 API requests',
    ['method', 'endpoint']
)
...
# Explicitly define the metrics endpoint
@app.route('/metrics')
def metrics_endpoint():
    try:
        # Update memory metrics before serving
        process = psutil.Process(os.getpid())
        memory_gauge.labels(service='iep3').set(process.memory_info().rss)
